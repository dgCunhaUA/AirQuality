...
class AirQualityServiceTest {
    ...
    void whenGetCityAirQualityByWrongName_ThenReturnsNotFoundMessage() throws IOException, InterruptedException {
        ...
        Map<City, AirQuality> response = airQualityService.getCurrentAirQualityByCity("WrongName");
        assertThat(response).isInstanceOf(HashMap.class);
        assertEquals(response.toString(), expectedResponse.toString());
    }
    ...
    @Test
    void whenGetCityAirQualityByLatLng_ThenReturnsAirQuality() throws IOException, InterruptedException {

        String response_body = "{\"message\":\"success\",\"stations\":[{\"_id\":\"60363d6c8f2bb86af93ba8fb\",\"placeId\":\"591a22fa84590ec0d7381c22d8b2d0849e4c3dd7f645170b3f6bebe1fab31ba8\",\"CO\":0.20208333333333334,\"NO2\":7.836,\"OZONE\":3.449,\"PM10\":24.814,\"PM25\":6.8,\"SO2\":3.384,\"city\":\"Viseu\",\"countryCode\":\"PT\",\"division\":\"Viseu\",\"lat\":40.661,\"lng\":-7.9097,\"placeName\":\"Viseu\",\"postalCode\":\"3500-004\",\"state\":\"Viseu\",\"updatedAt\":\"2021-04-23 11:00:00\",\"AQI\":28,\"aqiInfo\":{\"pollutant\":\"PM2.5\",\"concentration\":6.8,\"category\":\"Good\"}}]}\n";

        when( ambeeRepository.getCurrentAirQualityByLatLng("40.661", "-7.9097") ).thenReturn(response_body);
        assertEquals( ambeeRepository.getCurrentAirQualityByLatLng("40.661", "-7.9097"), response_body );
        assertThat( ambeeRepository.getCurrentAirQualityByLatLng("40.661", "-7.9097") ).isInstanceOf(String.class);
        ...
        Map<City, AirQuality> response = airQualityService.getCurrentAirQualityByLatLng("40.661", "-7.9097");
        assertThat(response).isInstanceOf(HashMap.class);
        assertEquals(response.toString(), expectedResponse.toString());
    }
    ...
    @Test
    void whenGetCityAirQualityByWrongLatLng_ThenReturnsNotFoundMessage() throws IOException, InterruptedException {

        String response_body = "{\"message\":\"Sorry, we couldn't find the specified place\",\"data\":[]}";

        when( ambeeRepository.getCurrentAirQualityByLatLng("----", "----") ).thenReturn(response_body);

        assertEquals( ambeeRepository.getCurrentAirQualityByLatLng("----", "----"), response_body );
        ...
        Map<City, AirQuality> response = airQualityService.getCurrentAirQualityByLatLng("----", "----");
        assertThat(response).isInstanceOf(HashMap.class);
        assertEquals(response.toString(), expectedResponse.toString());
    }
}